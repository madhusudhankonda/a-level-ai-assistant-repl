...
def generate_answer_feedback(question_image, answer_image, subject):
    """
    Generate feedback for a student's answer to a question using OpenAI's GPT-4.1 model
...
"""
    ...
    try:
        ...
        response = openai.chat.completions.create(
            model="gpt-4.1-preview",  # Updated to use GPT-4.1 preview model
            messages=[
...
def generate_explanation(base64_image, subject):
    """
    Generate an explanation for a question using OpenAI's GPT-4.1 model
