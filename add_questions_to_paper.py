#!/usr/bin/env python
"""
Script to add questions to a specific paper ID.
Usage: python add_questions_to_paper.py paper_id
Example: python add_questions_to_paper.py 73
"""
...
def main():
    """Main function to add questions to a paper"""
    if len(sys.argv) < 2:
        print("Usage: python add_questions_to_paper.py paper_id")
        return 1
    
    try:
        paper_id = int(sys.argv[1])
        questions_added, mark_schemes_added = create_question_set(paper_id, num_questions=5)
        
        print(f"Added {questions_added} questions and {mark_schemes_added} mark schemes to paper {paper_id}")
        return 0
    except ValueError:
        print(f"Invalid paper ID: {sys.argv[1]}")
        return 1
    except Exception as e:
        logger.exception(f"Error adding questions to paper: {str(e)}")
        return 1
